        ...
        <div class="col-md-12">
            @can('result_assessment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.result-assessments.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $resultAssessment->id }}">
                                        ...
                                        <td>
                                            @can('result_assessment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.result-assessments.show', $resultAssessment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('result_assessment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.result-assessments.edit', $resultAssessment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('result_assessment_delete')
                                                <form action="{{ route('frontend.result-assessments.destroy', $resultAssessment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('result_assessment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.result-assessments.massDestroy') }}",
