                    ...
                    <form method="POST" action="{{ route("frontend.tracer-stakeholders.update", [$tracerStakeholder->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\TracerStakeholder::KESESUAIAN_BIDANG_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="kesesuaian_bidang_{{ $key }}" name="kesesuaian_bidang" value="{{ $key }}" {{ old('kesesuaian_bidang', $tracerStakeholder->kesesuaian_bidang) === (string) $key ? 'checked' : '' }}>
