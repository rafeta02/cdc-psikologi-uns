        ...
        <div class="col-md-12">
            @can('industry_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.industries.create') }}">
                            ...
                            <tbody>
                                @foreach($industries as $key => $industry)
                                    <tr data-entry-id="{{ $industry->id }}">
                                        ...
                                        <td>
                                            @can('industry_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.industries.show', $industry->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('industry_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.industries.edit', $industry->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('industry_delete')
                                                <form action="{{ route('frontend.industries.destroy', $industry->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('industry_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.industries.massDestroy') }}",
