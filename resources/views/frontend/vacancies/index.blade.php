        ...
        <div class="col-md-12">
            @can('vacancy_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.vacancies.create') }}">
                            ...
                            <tbody>
                                @foreach($vacancies as $key => $vacancy)
                                    <tr data-entry-id="{{ $vacancy->id }}">
                                        ...
                                        <td>
                                            @can('vacancy_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.vacancies.show', $vacancy->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('vacancy_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.vacancies.edit', $vacancy->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('vacancy_delete')
                                                <form action="{{ route('frontend.vacancies.destroy', $vacancy->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('vacancy_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.vacancies.massDestroy') }}",
