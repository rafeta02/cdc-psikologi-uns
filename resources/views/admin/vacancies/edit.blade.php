...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.vacancy.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vacancies.update", [$vacancy->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.vacancy.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $vacancy->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id">
                    @foreach($companies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $vacancy->company->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $vacancy->description) !!}</textarea>
            ...
            <div class="form-group">
                <label for="open_date">Date Close Exist ?</label>
                <div class="form-check {{ $errors->has('close_date_exist') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="close_date_exist" value="0">
                    <input class="form-check-input" type="checkbox" name="close_date_exist" id="close_date_exist" value="1" {{ $vacancy->close_date_exist || old('close_date_exist', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <label for="persyaratan_umum">Requirements</label>
                <textarea class="form-control ckeditor {{ $errors->has('persyaratan_umum') ? 'is-invalid' : '' }}" name="persyaratan_umum" id="persyaratan_umum">{!! old('persyaratan_umum', $vacancy->persyaratan_umum) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('job_description') ? 'is-invalid' : '' }}" name="job_description" id="job_description">{!! old('job_description', $vacancy->job_description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('registration') ? 'is-invalid' : '' }}" name="registration" id="registration">{!! old('registration', $vacancy->registration) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('location') ? 'is-invalid' : '' }}" name="location_id[]" id="location_id" multiple>
                    @foreach($vacancy->locations as $location)
                        <option value="{{ $location->id }}" selected>{{ $location->name ?? '' }}</option>
                    @endforeach
                    {{-- @foreach($locations as $id => $location)
                        <option value="{{ $id }}" {{ (in_array($id, old('locations', [])) || $vacancy->locations->contains($id)) ? 'selected' : '' }}>{{ $location }}</option>
                    @endforeach --}}
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $vacancy->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('education') ? 'is-invalid' : '' }}" name="education[]" id="education" multiple>
                    @foreach($education as $id => $education)
                        <option value="{{ $id }}" {{ (in_array($id, old('education', [])) || $vacancy->education->contains($id)) ? 'selected' : '' }}>{{ $education }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('departments') ? 'is-invalid' : '' }}" name="departments[]" id="departments" multiple>
                    @foreach($departments as $id => $department)
                        <option value="{{ $id }}" {{ (in_array($id, old('departments', [])) || $vacancy->departments->contains($id)) ? 'selected' : '' }}>{{ $department }}</option>
                    @endforeach
                </select>
