...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.monitoringMagang.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.monitoring-magangs.update", [$monitoringMagang->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mahasiswa') ? 'is-invalid' : '' }}" name="mahasiswa_id" id="mahasiswa_id">
                    @foreach($mahasiswas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mahasiswa_id') ? old('mahasiswa_id') : $monitoringMagang->mahasiswa->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('magang') ? 'is-invalid' : '' }}" name="magang_id" id="magang_id">
                    @foreach($magangs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('magang_id') ? old('magang_id') : $monitoringMagang->magang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pembimbing') ? 'is-invalid' : '' }}" type="text" name="pembimbing" id="pembimbing" value="{{ old('pembimbing', $monitoringMagang->pembimbing) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal') ? 'is-invalid' : '' }}" type="text" name="tanggal" id="tanggal" value="{{ old('tanggal', $monitoringMagang->tanggal) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tempat') ? 'is-invalid' : '' }}" type="text" name="tempat" id="tempat" value="{{ old('tempat', $monitoringMagang->tempat) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('hasil') ? 'is-invalid' : '' }}" name="hasil" id="hasil">{!! old('hasil', $monitoringMagang->hasil) !!}</textarea>
                @if($errors->has('hasil'))
